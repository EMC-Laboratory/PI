...
num_loops  = 5
sol_found_array = np.ndarray((50,1), dtype= int)

# second argument is the # of ports
final_solutions = np.ndarray((num_loops,np.shape(z)[1] - 1),dtype= int)
conv_save_path = "Compare Methods Old GA Conv"
sols_save_path = "Compare Methods Old GA Sol"
os.makedirs(conv_save_path)
os.makedirs((sols_save_path))

test_file_name = "25 Capacitors Board 1, Old GA"
if os.path.isfile(test_file_name + ".npz"):
    raise FileExistsError("The file for writing decap solutions, {}, already exists".format(test_file_name))
...
for i in range(num_loops):
    ...
    print("R =", np.load('Supplement MidPts RL R 2 = 6mOhms to 10mOhms 2 Groups Zmax = 20 to 25mOhms 5 groups 5 Per Group.npz')['R_Ohms'][i])
    print("Zmax =", np.load('Supplement MidPts RL R 2 = 6mOhms to 10mOhms 2 Groups Zmax = 20 to 25mOhms 5 groups 5 Per Group.npz')['Zmax_Ohms'][i])
    print("L =", np.load('Supplement MidPts RL R 2 = 6mOhms to 10mOhms 2 Groups Zmax = 20 to 25mOhms 5 groups 5 Per Group.npz')['L_Henries'][i])
    ...
    f_conv_name = "Conv Final Check Supplement {}".format(i)
    ...
    if record_solutions:
        record_solutions = True
        f_sols_name = "Solutions Final Check Supplement {}".format(i)
    ...
    model.run()
    f_conv.write("Time taken =" + str(time.time() - loop_start_time) + "\n")
    ...
    final_solutions[i,:] = np.copy(decap_solution)
    sol_found_array[i] = 1 if target_met_bool == 'True' else 0


total_time = time.time() - start
np.savez(test_file_name, sols = final_solutions, time = total_time, sol_found = sol_found_array)
load_test = np.load(test_file_name + ".npz")['sols']
load_time = np.load(test_file_name + ".npz")['time']
