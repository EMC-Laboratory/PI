# -*- coding: utf-8 -*-
"""
Created on Sat Sep 26 18:39:59 2020
...
def gen_random_brd_shape(max_len, img_size, num_locations=51, n=8, thre_angle=30,
    ...
    top_loc_number = random.randint(2,num_locations-1)
    ...
    # Jack attempting to add random vrm location
    vrm_local_local_indx = random.choice(top_loc_number, 1, replace=False)
    vrm_local_indx = top_indx[vrm_local_local_indx]
    vrm_indx = ix[vrm_local_indx]
    print('vrm', vrm_indx)
    # print(vrm_local_local_indx)
    # print(ic_local_local_indx)
    # ix = ix[np.delete(top_indx, vrm_local_local_indx)]
    ...
    brd_shape_vrm = deepcopy(img)
    top_decap_loc = np.zeros(img.shape)
    bot_decap_loc = np.zeros(img.shape)
    brd_shape_ic[i[ic_indx],j[ic_indx]] = 2
    brd_shape_vrm[i[vrm_indx],j[vrm_local_indx]] = 3

    top_decap_loc[i[top_decap_indx],j[top_decap_indx]] = 1
    bot_decap_loc[i[bot_decap_indx],j[bot_decap_indx]] = 1
    ...
    return brd_shape_ic, brd_shape_vrm, top_decap_loc, bot_decap_loc, sxy
...
def gen_random_stackup(min_t, max_t, min_dt, min_n_layer, max_n_layer):
    ...
    n_tot = int(t/min_dt)
    n_layer = random.randint(min_n_layer, max_n_layer)
    ...
    power_layer_indx = random.randint(1, n_layer-2)
...
def gen_brd_data(brd, max_len, img_size, num_locations=20, n=8, thre_angle=30, rad=0.1, edgy=0.05, 
                 num_pts=10, min_t=1e-3, max_t=10e-3, min_dt=0.1e-3, min_n_layer=4, max_n_layer=10, 
                 er=4.4, via_dist=2e-3):
    brd_shape_ic, vrm_shape_ic, top_decap_loc, bot_decap_loc, sxy = gen_random_brd_shape(max_len=max_len, img_size=img_size)
    ...
    top_decap_x_indx, top_decap_y_indx = np.nonzero(top_decap_loc)
    bot_decap_x_indx, bot_decap_y_indx = np.nonzero(bot_decap_loc)
    ic_xy_indx = np.array([ic_x_indx, ic_y_indx])
    top_decap_xy_indx = np.ndarray((top_decap_x_indx.shape[0],2))
    bot_decap_xy_indx = np.ndarray((bot_decap_x_indx.shape[0],2))
    top_decap_xy_indx[:,0] = top_decap_x_indx
    top_decap_xy_indx[:,1] = top_decap_y_indx
    bot_decap_xy_indx[:,0] = bot_decap_x_indx
    bot_decap_xy_indx[:,1] = bot_decap_y_indx

    # Jack = map the vrm location
    vrm_x_indx, vrm_y_indx = np.where(vrm_shape_ic == 3)
    vrm_xy_indx = np.array([vrm_x_indx, vrm_y_indx])
    vrm_loc = random.randint(0,1)

    # now all the cases, the power and gnd via pair are horizontally oriented
    brd.ic_via_xy = np.ndarray((2,2))
    brd.ic_via_xy[0,0] = (ic_x_indx+0.5)*unit_size - via_dist/2
    brd.ic_via_xy[0,1] = (ic_y_indx+0.5)*unit_size
    brd.ic_via_xy[1,0] = (ic_x_indx+0.5)*unit_size + via_dist/2
    brd.ic_via_xy[1,1] = (ic_y_indx+0.5)*unit_size
    brd.ic_via_type = np.array([1,0])

    # matrix that mark the location of the ports
    brd.decap_via_xy = np.zeros(((top_decap_x_indx.shape[0] + bot_decap_x_indx.shape[0])*2, 2))
    brd.decap_via_type = np.zeros(((top_decap_x_indx.shape[0] + bot_decap_x_indx.shape[0])*2))
    brd.decap_via_loc = np.zeros(((top_decap_x_indx.shape[0] + bot_decap_x_indx.shape[0])*2))
    ...
    z = brd.calc_z()
    return z, brd_shape_ic, ic_xy_indx, vrm_xy_indx, vrm_loc, top_decap_xy_indx, bot_decap_xy_indx, stackup, die_t, t, d2top, sxy, brd.ic_via_xy, brd.ic_via_type, brd.decap_via_xy, brd.decap_via_type, brd.decap_via_loc, max_len
...
if __name__ == '__main__':
    ...
    for i in range(0,N):
        print(str(n))
        t1 = time.time()
        z, brd_shape_ic, ic_xy_indx, vrm_xy_indx, vrm_loc, top_decap_xy_indx, bot_decap_xy_indx, stackup, die_t, t, d2top, sxy, ic_via_xy, ic_via_type, decap_via_xy, decap_via_type, decap_via_loc, max_len = gen_brd_data(brd=brd, max_len=200e-3, img_size=16)
        ...
        np.savez(os.path.join(BASE_PATH, str(n)+'.npz'), z=z, brd_shape_ic=brd_shape_ic, 
                 ...
                 bot_decap_xy_indx=bot_decap_xy_indx, stackup=stackup, die_t=die_t, 
                 ...
                 max_len=max_len, vrm_xy_indx=vrm_xy_indx, vrm_loc = vrm_loc)
        n += 1
        print("Number of layers is " + str(stackup.shape[0]) + ", number of ports is " 
              + str(int(ic_via_xy.shape[0]/2+decap_via_xy.shape[0]/2)))
        print("Consumed time is " + str(time.time()-t1) + " s")
        print("Total consumed time is " + str(time.time()-t0) + " s")
        print("Number of error cases is " + str(num_error))

        #Jack - Ok so i added in a vrm location to this. I now have no clue where I can connect the VRM
        #       How do I map a vrm xy to a port #?
        print("\n")

# vrm_local_local_indx = random.choice(top_loc_number, 1, replace=False)
# vrm_local_indx = top_indx[ic_local_local_indx]
# vrm_indx = ix[vrm_local_indx]
#
# top_decap_indx = ix[np.delete(top_indx, ic_local_local_indx)]
# bot_decap_indx = np.delete(ix, top_indx)
