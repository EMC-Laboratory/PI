...
def initial_sol_score(decap_maps_z, opt):
    ...
    for map_num, map_z in enumerate(decap_maps_z):
        ...
        decap_map_scores[map_num] = round(1/(average_percent_greater-1) + 1/(average_from_center - 1) + 1/(max_z_above  - min_z))

    return decap_map_scores



def calc_score(decap_maps, decap_maps_z, opt, best_score, min_zero_map, shift_pt):

    # ALL THESE %FROM WHATEVER STUFF NEEDS to bE STANDARDIZED SO TROUBLE SHOOTING ISNT A PAIN
    decap_map_scores = [None] * len(decap_maps_z)
    ...
    for map_num, map_z in enumerate(decap_maps_z):  # iterates through each solution in population
        ...
        region1_pt = round(marker/3)
        region2_pt = round(2*marker/3)
        region3_pt = marker

        low_target_area = np.trapz(opt.ztarget[0:region1_pt], opt.freq[0:region1_pt])
        low_solution_area = np.trapz(abs_map_z[0:region1_pt], opt.freq[0:region1_pt])
        low_area_between = abs(low_target_area - low_solution_area)

        mid_target_area = np.trapz(opt.ztarget[region1_pt:region2_pt], opt.freq[region1_pt:region2_pt])
        mid_solution_area = np.trapz(abs_map_z[region1_pt:region2_pt], opt.freq[region1_pt:region2_pt])
        mid_area_between = abs(mid_target_area - mid_solution_area)

        hi_target_area = np.trapz(opt.ztarget[region2_pt:region3_pt], opt.freq[region2_pt:region3_pt])
        hi_solution_area = np.trapz(abs_map_z[region2_pt:region3_pt], opt.freq[region2_pt:region3_pt])
        hi_area_between = abs(hi_target_area - hi_solution_area)

        low_area_mod = low_target_area/low_area_between
        # mid_area_mod = midsolutionarea/mid_area_between not bad
        mid_area_mod =  mid_solution_area/mid_area_between
        high_area_mod = hi_target_area/hi_area_between

        # ---------------------looking at quality of max and mins-------------

        maxs = pdn1.find_maxs(abs_map_z)
        mins = pdn1.find_mins(abs_map_z, True)
        min_z_ind = np.argmin(abs_map_z)
        # decrease_percent = 0
        # avg_decrease_percent = 0
        # test_mod = 0
        # if len(maxs) == 0 or len(mins) == 0:
        #     print('No local mins or max')
        #     pass
        # else:
        #     max_first = True if maxs[0] < mins[0] else False
        #     decrease_percent = 0
        #
        #     if max_first is True: # max first is fine
        #         counter = 0
        #         # sure theres a way to do this without the counter, but for clarity and 1st time, ill use one
        #         for min, max in zip(mins, maxs):
        #             if min != min_z_ind:
        #                 decrease_percent = decrease_percent + abs_map_z[min]/abs_map_z[max]
        #                 counter = counter + 1
        #         avg_decrease_percent = decrease_percent / counter if counter > 0 else 0
        #     else: # min first is maybe fine(??????)
        #         mins.pop(0)
        #         counter = 0
        #         if len(mins) > 0:
        #             for min, max in zip(mins, maxs):
        #                 if min != min_z_ind:
        #                     print('values', abs_map_z[min], abs_map_z[max])
        #                     decrease_percent = decrease_percent + abs_map_z[min] / abs_map_z[max]
        #                     counter = counter + 1
        #             avg_decrease_percent = decrease_percent / counter if counter > 0 else 0
        #             # try:
                    #     mins.remove(min_z_ind)
                    #     decrease_percent = decrease_percent / len(mins) if len(mins) > 0 else decrease_percent
                    # except ValueError:
                    #     print('Value Errored')
                    #     decrease_percent = decrease_percent / len(mins) if len(mins) > 0 else decrease_percent
        # test_mod = avg_decrease_percent if (avg_decrease_percent <= .5 and avg_decrease_percent > 0) else 0
        average_min_distance = sum(mins)/len(mins) if len(mins) != 0 else 1;

        # average from center
        holder = 0
        solution_average = sum(abs_map_z) / len(map_z)
        for i in abs_map_z:
            if i > solution_average:
                holder = holder + solution_average / i
            else:
                holder = holder + i / solution_average
        average_from_center = holder / len(map_z)
        # the percentage tells you how off from the center you are percentage wise
        port_mod = port_modifiers(decap_maps[map_num])
        port_mod = 0



        score =  low_area_mod + mid_area_mod + high_area_mod + 1/(1-average_from_center) #+ port_mod
        score = (num_empty + 10)

        #score = 1/(1-average_from_center) + port_mod + num_empty
        ...
        # I still have a problem with this
            ...
            if replace_chance == 0:
                ...
                decap_maps[map_num] = add_zeroes(decap_maps[map_num], min_decap_num)
                score = round(best_score / 3) + 1
                decap_map_scores[map_num] = score
            else:
                score = (score * 1/2)
                score = score if (score < (best_score / 2) + 1) else ((best_score / 2) + 1)
                decap_map_scores[map_num] = score

    return decap_map_scores, new_min_zero_map

def port_modifiers(decap_map):
    #port 8 is 23
    #ports 3,7,12 is 18,22,27
    far_port_list = [2,6,11]
    near_port_list = [7]
    counter = 0
    near_port_mod = 0
    far_port_mod = 0

    for i in far_port_list:
        if decap_map[i] == 0:
            pass
        else:
            far_port_mod = far_port_mod + .5

    for i in near_port_list:
        if decap_map[i] == 0:
            pass
        else:
            near_port_mod = near_port_mod + .5
    return near_port_mod + far_port_mod
...
def get_min_map(min_map, test_map):
    ...
    if len(test_min_map) == 0 or (len(test_min_map) - np.count_nonzero(test_min_map)) < (len(test_map) - np.count_nonzero(test_map)):
...
def cross_over(decap_maps, Population, opt):
    ...
    # I want to shuffle the order of the parents to see if i can encourage more interesting crossovers.

    #could be simplified. definitely
    if len(cross_parents) >= 2:  # Two parents at least must exist for crossover to be done
        for i, parent_pos in enumerate(cross_parents):
            ...
            if i + 1 == len(cross_parents):
                ...
                if left_parent == 1:
                    new_chromo = [decap_maps[parent_pos][j] for j in range(point)]
                    new_chromo = new_chromo + [decap_maps[cross_parents[i + 1]][j] for j in range(point, opt.decap_ports)]
                    cross_pop[parent_pos] = new_chromo
                else:
...
def elitism(best_score, best_map, decap_maps, decap_map_scores, initial_sol_found, min_zero_map, other_scores = True):
    # current elitism implementation I think hurts the time it takes to find initial solutions 2/25
    ...
    if other_scores is True and initial_sol_found is True:
        best_indices = [index for index, score in enumerate(decap_map_scores) if
                        score >= (best_score * .8) or score >= (max(decap_map_scores) * .8)]
        # elitism not just the best sol, can't tell if this is stupid or not
    else:
        best_indices = [index for index,score in enumerate(decap_map_scores) if (score == best_score or score == max(decap_map_scores))]

    max_allowed_copies = 10 if initial_sol_found is True else len(decap_maps) # no cap if looking for initial sol

    if len(best_indices) < max_allowed_copies:
        replace_sols = [decap_maps[i] for i in best_indices]
        # solutions used to replace

        replace_indices = random.sample(range(len(final_pop)), max_allowed_copies - len(best_indices))
        # indices of the population that will have their map overwritten
        ...
        #indexs of the actual solutions used to replace
        ...
        print('Elitism Did Not Occur. Enough Copies Present in Population.', len(best_indices),'copies present.  '
                                                                                        'The limit is', max_allowed_copies)
