...
class PDN():
    ...
    def init_para(self):
        ...
        self.decap_list = []
        decap_0, _ = short_1port(rf.Network('decap2/0_GRM033C80J104KE84.s2p').interpolate(self.freq))
        self.decap_list.append(decap_0.z)
        decap_1, _ = short_1port(rf.Network('decap2/1_GRM033R60J474KE90.s2p').interpolate(self.freq))
        self.decap_list.append(decap_1.z)
        decap_2, _ = short_1port(rf.Network('decap2/2_GRM155B31C105KA12.s2p').interpolate(self.freq))
        self.decap_list.append(decap_2.z)
        decap_3, _ = short_1port(rf.Network('decap2/3_GRM155C70J225KE11.s2p').interpolate(self.freq))
        self.decap_list.append(decap_3.z)
        decap_4, _ = short_1port(rf.Network('decap2/4_GRM185C81A475KE11.s2p').interpolate(self.freq))
        self.decap_list.append(decap_4.z)
        decap_5, _ = short_1port(rf.Network('decap2/5_GRM188R61A106KAAL.s2p').interpolate(self.freq))
        self.decap_list.append(decap_5.z)
        decap_6, _ = short_1port(rf.Network('decap2/6_GRM188B30J226MEA0.s2p').interpolate(self.freq))
        self.decap_list.append(decap_6.z)
        decap_7, _ = short_1port(rf.Network('decap2/7_GRM219D80E476ME44.s2p').interpolate(self.freq))
        self.decap_list.append(decap_7.z)
        decap_8, _ = short_1port(rf.Network('decap2/8_GRM31CR60J227ME11.s2p').interpolate(self.freq))
        self.decap_list.append(decap_8.z)
        decap_9, _ = short_1port(rf.Network('decap2/9_GRM32EC80E337ME05.s2p').interpolate(self.freq))
        self.decap_list.append(decap_9.z)

        self.seg_bd()
        
    def init_decap_list(self):
        decap_list = []
    ...
    # interpolation
        ...
        for i in range(0,bxy.shape[0]-1):
            ...
            if dl < len_ith:
                ...
                #nseg += np.floor(len_ith/dl) #bug here?, always zero
                nseg += 1
    ...
    # only support one inner boundary (one void)
        ...
        if self.inner_bd_node.size == 0 and self.inner_sxy.size == 0:
            ...
            self.area = PolyArea(self.sxy[:,0],self.sxy[:,1])
        elif self.inner_bd_node.size == 0:
            self.sxy = np.concatenate((self.outer_sxy,self.inner_sxy))
            self.area = (PolyArea(self.outer_sxy[:,0],self.outer_sxy[:,1]) 
        ...
        for k in range(0,Ntot):
            for m in range(0,Ntot):
                if m >= self.sxy.shape[0]  and k!=m:
                    # source cell is ic cell
                    # observing cell is every cell except source
                    self.D[k,m] = 0
                elif m >= self.sxy.shape[0] and k==m:
                    self.D[k,m] = -1
                    # source cell is ic cell
                    # observing cell is ic cell
                elif m < self.sxy.shape[0] and k >= self.sxy.shape[0]:
                    # source cell is a boundary cell
                    # observing cell is an ic cell
                    ...
                    # source cell is another boundary cell
                    # observing cell is a boundary cell
                    ...
                    ra = sqrt((xa-xk)**2+(ya-yk)**2)
                    if np.abs(v2)<1e-10:
                        # essentially to catch the parallel cases where ra is in same dir as v1
                        self.D[k,m] = 0
                    else:
                        # theres a bug here.
                        # v2 can be > 1e-10 and ra**2 - v1**2 = 0.
                        # better to check ra == vi equality maybe in the if statment?.
                        self.D[k,m] = v2/pi*(1/sqrt(ra**2-v1**2)*(atan((-v1+lm)/sqrt(ra**2-v1**2))+
                         atan(v1/sqrt(ra**2-v1**2)))-lm/(self.R**2))
        for k in range(0,Ntot):
            for m in range(0,Ntot):
                if m >= self.sxy.shape[0] and k==m:
                    # This is along the port boundary
                    # I cannot find where this expression comes from
